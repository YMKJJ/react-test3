...
// ------------ 거의 사용하지 않음 ---------------------------
// useEffect : useState()와 함께 가장 많이 사용되는 훅, 생명주기 함수인
//             componentDidMount, componentDidUpdate, componentWillmount의 기능을 하나로 합한기능
...
// useState(초기값) : 확장 표현식을 통해서 지정한 변수를 state 객체로 설정함, 초기값은 state로 설정된 변수의 초기값을 말함.
// state로 지정할 변수가 여러개일 경우 useState()를 변수 수만큼 실행해야 함

// useEffect : 클래스 컴포넌트에 존재하는 생명주기 함수를 사용할 수 있도록 함
// 사용법 :
// useEffect(이펙트 함수(콜백함수), 의존성 배열);
// 의존성 배열 : 콜백 함수인 이펙트 함수가 의존하고 있는 배열, 해당 의존성 배열안에 있는 변수 중 하나라도 값이 변경되면 실행
// 이펙트 함수는 기본적으로 컴포넌트가 처음 렌더링된 후 데이터 업데이트에 의한 재렌더링 시 실행
// 이펙트 함수를 마운트, 언마운트 시 각각 1번씩만 실행하고자 할 경우 의존성 배열에 빈배열을 사용(componentDidMount, componentDidUnmount 기능 수행)
...
function CountBtn2(props){
    ...
    return(
        <div>
            ...
            <button className={"btn btn-secondary"} onClick={countDown}> - </button>
        </div>
    )
}
