...
// Provider를 이용하여 생성된 컨텍스트를 사용할 컴포넌트를 지정
// 사용법 :
// const 컨텍스트명 = React.createContext(기본값);

// Context.Provider : 생성된 컨텍스트를 사용할 자식 컴포넌트를 지정 시 사용
// value 속성에 전달하고자 하는 데이터를 지정, 해당 데이터를 사용하고자 하는 자손 컴포넌트를
// 감싸면 어느 위치의 자손 컴포넌트이든 상관없이 해당 데이터에 접근이 가능함.
// 여러개 사용가능
// 사용법 :
...
// Context.Consumer : Provider로 감싸진 자손 컴포넌트 중 컨텍스트에 저장된 데이터를 사용하고자할 경우 Consumer을
// 사용하여 지정한 데이터를 가져옴
// 함수 컴포넌트에서 사용 여러개를 사용하는 것이 가능함
// 사용법 :
// <컨텍스트명.Consumer>
//      {value => <컴포넌트 key={value} />}
// </컨텍스트명.Consumer>

// Class.contextType : 클래스 컴포넌트에서 컨텍스트에 저장된 내용을 가져올 경우 사용함.
// 사용법 :
// componentDidMount() {
